...
Sample X: $s^2=$ `r fitb(3)`

Sample Y: $s^2=$ `r fitb(13)`
:::
...
* The variances for different data sets are almost impossible to compare. The reason for that is best seen by realising that the value of the variance will change if you multiply each value by 2\footnote{You could try and confirm that in this case the variance will, in fact, increase by the factor 4.}. However, if we multiply each observation by two, then really, nothing has changed about the dispersion of our data.
...
![Variance and standard deviation.](images/PopSampleScheme.png)
...
n = 5
temp_sample <- runif(5)
temp_sample <- round(temp_sample*100,0)
...
Mean, $\mu$ = `r fitb(round(mean(temp_sample),4))`

Variance, $\sigma^2$ = `r fitb(round(var.p,4))`

Standard deviation, $\sigma$ =  `r fitb(round(sigma,4))`

Assuming that these observations are a **sample**, calculate the following (all to 4 dp):

Mean, $\bar{x}$ = `r fitb(round(mean(temp_sample),4))`

Variance, $s^2$ = `r fitb(round(var.s,4))`

Standard deviation, $s$ =  `r fitb(round(sd,4))`


A veterinarian weighed a sample of 6 puppies. Here are each of their weights (in kilograms): 1, 2, 7, 7, 10, 15.

The mean of these weights is `r fitb(7)` $kg$

The variance is `r fitb(26.80)` $kg^2$

The standard deviation is `r fitb(5.18)` $kg$

`r hide("Feedback")`

[Solution video, YouTube  (7min)](https://youtu.be/9jjOrKp8BKg).
...
## Measures of skewness and higher order moments
...
![Symmetric and skewed distributions.](images/SkewedDist.png)

::: callout-important

#### Exercise

The income distribution in the UK is `r mcq(c(answer = "symmetric", answer = "right skewed", "left skewed"))`.
...
![Symmetric and skewed distributions.](images/SymmetricDist.png)
...
![Left Skewed distribution.](images/LeftSkewedDist.png)
...
In the UK income distribution the mean income is `r mcq(c("smaller than", "equal", answer = "larger than"))` the median income?
...
![UK IncomeDistribution 2019.](images/UK_IncomeDistribution_2019.png)
...
A lot of effort has gone into measuring the amount of inequality in different countries and across time.  An excellent example of this is the [Chartbook of Economic Inequality](https://www.chartbookofeconomicinequality.com/). This has been set up by [Anthony Atkinson](https://www.tony-atkinson.com/) (1944-2017) who was a true giant in economic research into inequality.
...
![Measures of income inequality, UK. Source: The Chartbook of Economic Inequality.](images/UKinequality.png)
...
Find on the website of [The Chartbook of Economic Inequality](https://www.chartbookofeconomicinequality.com) the equivalent country chart for Malaysia.

```{r}
#| echo: false
opts_p <- c(
   "The general development of inequality in Malaysia (since 1950) is similar to that in the UK.",
   answer = "The level of poverty has decreased fairly steadily.",
   answer = "Top earners have not increased their share of income.",
   answer = "In general Malaysia has decreasing inequality.",
   "The Gini coefficients of the UK and Malaysia indicate that the Malaysian income distribution is more equal than that of the UK."
)
```
...
* An example for the [calculation of the population variance](https://www.khanacademy.org/math/probability/descriptive-statistics/variance\_std\_deviation/v/variance-of-a-population)
* [Why do we need the sample variance?](https://www.khanacademy.org/math/probability/descriptive-statistics/variance\_std\_deviation/v/sample-variance)
* These two clips illustrates nicely why the sample variance uses the factor $1/(n-1)$. [First:](https://www.khanacademy.org/math/probability/descriptive-statistics/variance\_std\_deviation/v/review-and-intuition-why-we-divide-by-n-1-for-the-unbiased-sample-variance) giving an intuitive explanation of why we need the factor $1/(n-1)$. [Second: ](https://www.khanacademy.org/math/probability/descriptive-statistics/variance\_std\_deviation/v/simulation-showing-bias-in-sample-variance) useing a simulation to convince you.
