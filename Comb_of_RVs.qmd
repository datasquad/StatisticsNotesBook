...
\subsection{Means and variances: a recap}
...
the last term illustrating that the transformation is a linear one with a multiplicative factor $\frac{1}{\sigma}$ (like a slope coefficient)and a shift factor $-\frac{\mu}{\sigma}$ (like a constant). Let's learn from that example how the properties of a linear transformation of a rv are related to those of the original rv. 
